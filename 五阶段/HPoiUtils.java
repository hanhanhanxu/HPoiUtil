 ...
 *
 * 在读取的时候分两步：读取string数据，转化为对象数据
 * 现在的读取策略是读取全部数据，包含第一行（不管传入true还是false），包含空数据，读取为空会转化为空串 ""。Excel的读取按照listField的长度，CSV的读取为读取一行
 * 在转化的时候根据true或false判断是否转化首行，转化时只根据listField的内容转化，不存在于listField中但是Class中有的字段不会被设置，默认为NULL
 */
public class HPoiUtils {
    ...
    public static List exeCsv(File file, List listField, Class c, boolean hasHeader) {
        //参数检查
        Boolean check = check(file, 0, listField, c); //这里传入的0是没有用的，只是为了检查可以复用之前的代码，因为之后获取字符串结合时不是调用getStr而是调用readCSV
        if(!check){
            return null;
        }
        //得到结果
    ...
    private static List<String> readCSV(File file){
        ...
        try ( BufferedReader bufferedReader = new BufferedReader(new FileReader(file)) ){
            //if (hasHeader) bufferedReader.readLine(); 读取的时候不控制，读取所有数据，在转化为对象的时候控制，不转化第一条记录
