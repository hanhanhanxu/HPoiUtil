...
public class HPoiUtils {
     ...
     * @param hasHeader Excel文件中是否有表头 true 是 false 否
     * @return 对象集合
     */
    public static List exe(File file, List<String> listField, boolean hasHeader) throws NotFoundException, CannotCompileException {
        //参数检查
        Boolean check = check(file, listField);
        if(!check){
            return null;
        }
        //生成类
        ...
        List listResultStr = getStr(file, len); //得到字符串数据集合
        List listResultObj = getObj(listField, listResultStr, c, hasHeader); //得到对象数据集合
     ...
     * 参数检查 动态生成类的版本
     * @param file Excel文件
     * @param listField 表头对应在类中的属性名称
     * @return 是否初始化成功
     */
    private static Boolean check(File file, List<String> listField){
        try {
            //参数校验
            //1、listField集合校验
     ...
     */
    private static Class generateClass(List<String> listField) throws NotFoundException, CannotCompileException {
        // 得到一个池
        ClassPool pool = ClassPool.getDefault();
        // 根据名称在运行期间动态生成一个类
        String className = JSDemo.class.getPackage().getName() + "." + CLASSNAME + ".java";
        // 根据类名 动态生成一个CtClass
     ...
     * @param c 对应Class
     * @param hasHeader Excel文件中是否有表头 true 是(第一行是表头) false 否（全是数据）
     * @return 对象集合
     */
    public static List exe(File file, List<String> listField, Class c, boolean hasHeader){
        //参数检查
        Boolean check = check(file, listField, c);
        if(!check){
            return null;
        }
        Integer len = listField.size();
        ...
        //得到字符串数据集合
        List listResultStr = getStr(file, len);
        //得到对象数据集合
        List listResultObj = getObj(listField, listResultStr, c, hasHeader);
        return listResultObj;
    }
     ...
     * 参数检查
     * @param file Excel文件
     * @param listField 属性名称
     * @param c 对应类
     * @return 是否初始化成功
     */
    private static Boolean check(File file, List<String> listField, Class c){
        try {
            //1、listField集合校验
            if (CollectionUtils.isEmpty(listField)) {
                System.out.println("listField集合为空, 表头元素对应属性名字的集合是必须的");
    ...
    private static List getStr(File file, Integer len){
        ...
        try {
            ...
            if("xls".equals(s)){
                listResultStr = doXls(in,len);
            }else if("xlsx".equals(s)){
                listResultStr = doXlsx(in,len);
            }else{
     ...
     * @throws IOException
     */
    private static List doXls(InputStream in, Integer len) throws IOException {
     ...
     * @throws IOException
     */
    private static List doXlsx(InputStream in, Integer len) throws IOException {
        List<String> listResultStr = new ArrayList();
    ...
    private static Object regression(String typeName,String dataStr){
        //如果是java.lang.Integer类型，但是数据data又存在小数点，那么就只取前面的部分 1.00 -> 1
